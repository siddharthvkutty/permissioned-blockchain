...
pending-transaction mempool, known peers, and a cached copy of the
validator rotation order fetched from the MCA.
...
class NodeStore:
    def __init__(self, path: str):
        ...
        self.data = {
            ...
            "cached_validators": [],  # last known validator rotation order from the MCA
        }
        self._load()
    ...
    def _load(self):
        if os.path.exists(self.path):
            ...
            self._save()
        self.data.setdefault("cached_validators", [])
    ...
    # ---------------------------------------------------------------- validators
    def get_cached_validators(self):
        return list(self.data.get("cached_validators", []))

    def set_cached_validators(self, ordered_addresses: list):
        with self.lock:
            self.data["cached_validators"] = list(ordered_addresses)
            self._save()
